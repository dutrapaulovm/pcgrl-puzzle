...
for (var i = 0; i < ww; i++;)
{
    ...
    {			
		...
		if file[# i, j] == 2
			instance_create_layer(xx+xx_space, yy+yy_space, "Level",objDoorEntrance )			
		else
		if file[# i, j] == 3
			instance_create_layer(xx+xx_space, yy+yy_space, "Level",objDoorExit )
		else		
		if file[# i, j] == 4
			instance_create_layer(xx+xx_space, yy+yy_space, "Items", objGold )			
		else
		if file[# i, j] == 5
			instance_create_layer(xx+xx_space, yy+yy_space, "Items", objKey )			
		else		
		if file[# i, j] == 6
			instance_create_layer(xx+xx_space, yy+yy_space, "Player",objPlayer )								
		else
		if file[# i, j] == 7
			instance_create_layer(xx+xx_space, yy+yy_space, "Enemies",objEnemy )											
		else
		if file[# i, j] == 8
			instance_create_layer(xx+xx_space, yy+yy_space, "PlayerWeapons",objWeapon )														
