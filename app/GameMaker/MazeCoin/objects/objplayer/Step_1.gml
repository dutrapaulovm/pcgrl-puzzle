...
if (kleft && !kright)
{
	move_x = m_grid * dir_x //(m_move_spd * delta_time * MS_TO_S60) * dir_x;
	flip = -1
...
if (kup && !kdown)
{
	move_y = m_grid * dir_y //(m_move_spd * delta_time * MS_TO_S60) * dir_y;
	m_state = STATE_PLAYER.run
}
else
	if (!kup && kdown) 
