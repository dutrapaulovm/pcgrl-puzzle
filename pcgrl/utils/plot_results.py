...
class PlotResults:
    ...
    def plot_stackbarvert(self, title, path, filename, n = 600, entropy_min = 1.80):
    ...
    def plot_stackbar(self, title, path, filename, n = 600):
    ...
    def plot_levels_repeted(self, title, path, filename, n = 600):
        labels = [key for key in self.data]       
    ...
    def plot(self, title, path, filename, n = 600, entropy_min = 1.80):
        category_names = ['H >= 1.80', 'H < 1.80']
        labels = [key for key in self.data]       
        best  = []
        worst = []
        ...
        #plt.figure(figsize=(16,5))
        graph_best = ax.bar(x - width/2, best, width, label='H >= {}'.format(entropy_min))
        graph_worst = ax.bar(x + width/2, worst, width, label='H < {}'.format(entropy_min))

        # Add some text for labels, title and custom x-axis tick labels, etc.
        #ax.set_ylabel('% of Levels generated')
        ax.set_ylabel('% de cenários gerados')
    ...
    def plot_entropy(self, path, filename, title):
        ...
        for key, value in self.data.items():                 
            ...
            if len(levels) > 0:
                ...
                plt.grid()
                plt.savefig(path+"/PDF-"+filename+"-"+key+".png")        
                plt.close()          
    ...
    def plot_scatter(self, path, filename, title, yfield, xfield, xlabel, ylabel):               
        ...
        for i in range(len(games)):
            ...
            plt.title(stitle)
            plt.savefig(path+"/"+filename+"-"+games[i]+".png")    
            #print(path+"/"+filename+"-"+games[i]+".png")
    ...
    def plot_boxplot(self, path, filename, title, xlabel = "Games"):
        ...
        ax.set_xticklabels(games, fontsize=14)
        #ax.set_title(stitle)
        ax.set_ylabel("Entropy Levels", fontsize=14)
    ...
    def plot_similarity_levels(self, path, filename):
        ...
        games = [Game.ZELDA.value, Game.MINIMAP.value, Game.MAZECOIN.value,  Game.MAZE.value, Game.DUNGEON.value]                    
        ...
        for key in exp:   
            ...
            for g in range(len(data)):                                 
                ...
                for i in range(0, 5): #len(segments)):
                    ...
                    title = 'Similarity of levels of ' + _game + ", Agent : " + key                 
                    plt.title(title)                                         
                    plt.savefig(path+"/"+filename+"-"+_game+"-"+key+"-Level"+str(i+1)+".png")
                    plt.close()
                    #time.sleep(0.5)                          
                    
        """                    
                    
    def csv_comparative(self, path, filename, n = 600, envs = None, entropy_min = 1.80):
        
        info = ["EntropyQTotal", "EntropyRelativeSuccess", "EntropyQ", "EntropyMinMax", "EntropiaAvgMean", "EntropyMean", "EntropyMedian", "EntropyStd", "EntropyVar", "ChangeMeanMedian", "ChangeMean", "ChangeAll", "ChangeMedian", "ChangeStd", "ChangeMedian", "ChangeVar", "Levels", "SegmentsMean", "SegmentsStd", "SegmentsMedian", "SegmentsVar", "SegmentsUsed"]#, "LevelsRepetead"]
        #info = ["LevelsRepeated"]
        ...
        for info in info:
            if envs is None:
                games = [Game.ZELDA.value, Game.MINIMAP.value, Game.MAZECOIN.value, Game.MAZECOINLOWMAPS.value, Game.MAZE.value, Game.DUNGEON.value, "Agent"]
            else:
                games = aux_envs
            ...
            for key in exp:   
                ...
                for g in range(len(data)):                 
                    ...
                    if (info == "EntropyQ"):
                        ...
                        dt_segments = pd.DataFrame({"segments" : segments})                                   
                        dt_segments = dt_segments.groupby(['segments'])['segments'].count()    
                        filename_dt = "{}-{}-{}-{}.csv".format(path, info, _game, filename)                                             
                        filename_dt = os.path.join(path, filename_dt)
                        dt_segments.to_csv(filename_dt)
    ...
    def plot_hist_changes(self, path, filename, title):
        ...
        for g in games:
            ...
            #ax.set_ylabel('Number of Levels')            
            ax.set_ylabel('Quantidade de Níveis')            
            ax.hist(col_segments, color='g', alpha=0.70, label=g)    
            avg = 0
            std = 0
            mode = 0
    ...
    def plot_hist(self, path, filename, title):
        ...
        for key, value in self.data.items():                 
            ...
            if 'entropy' in df.columns:           
                dtbest  = df.where(df["entropy"] >= 1.80)
                dtworst = df.where((df["entropy"] >= 0.0) & (df["entropy"] < 1.80))            
                best.append(dtbest["entropy"].count())
                worst.append(dtworst["entropy"].count())
            else:
                best.append(0)
                worst.append(0)                
    ...
    def plot_entropy_distance(self):
        ...
        ax.axvline(1.80, color="k", linestyle="dashed", linewidth=2, label="Min entropy: {} ".format(1.80))        
        ax.plot([1.80], [-2.458637729], 'ro', ms=8, mec='r')                        
        #ax.annotate('-2.458637729', xy=(1.80, -2.458637729), xytext=(1.90, -3.0), fontsize=12 )        
        ax.annotate('1.80', xy=(1.80, -2.458637729), xytext=(1.90, -3.0), fontsize=12 )        
